...
int main(int argc, const char * argv[]) {
    ...
    auto x = *std::min_element(vec.begin(), vec.end());
    
    std::cout << x << std::endl;
    
    
    auto y = std::accumulate(vec.begin(),vec.end(), 0);
    
    std::cout << y << std::endl;
    
    std::cout << "The number of even values is: " << std::count_if(vec.begin(), vec.end(), [](int num){return num % 2 == 0;}) <<std::endl;
