...
function App() {
    ...
    function handleLogin(ke, user, rm) {
        console.log("test 1 " + ke.target.id);
        if (ke.key === "Enter" || ke.target.id === "joinButton") {
    ...
    function handleMessageSend(ke, user, rm, message) {
        ...
        function handleSubmit() {
            userColors["userName"] = value;
            ws.current.send("color " + "userName" + " " + "room" + " " + value);
            console.log("test ws send");
        }

    return (
    <div>
        {home===1 ?  <InputWidget login={handleLogin}/> :
    <ChatPage chatRoomName={currentRoom.current} allMessages={allMessages} message={handleMessageSend}> </ChatPage>}
    <select
        id="ColorSelect"
        value = {value}
