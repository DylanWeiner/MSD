 ...
 */
PTR(Expr) VarExpr::subst(std::string name, PTR(Expr) newVal) {
    if(name == val) {
        return newVal;
    }
 ...
 * \param newVal is the value we will be replacing name with.
 */
PTR(Expr) NumExpr::subst(std::string name, PTR(Expr) newVal) {
...
void NumExpr::printExp(std::ostream &ot) {
    ot << CAST(NumVal)(CAST(NumExpr)(THIS)->interp())->to_string();
}
...
PTR(Val) AddExpr::interp() {
    return (CAST(AddExpr)(THIS)->lhs->interp()->add_to(CAST(AddExpr)(THIS)->rhs->interp()));
}
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) AddExpr::subst(std::string name, PTR(Expr) newVal) {
...
PTR(Val) MultExpr::interp() {
    return (CAST(MultExpr)(THIS)->lhs->interp()->mult_with(CAST(MultExpr)(THIS)->rhs->interp()));
}
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) MultExpr::subst(std::string name, PTR(Expr) newVal) {
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) LetExpr::subst(std::string name, PTR(Expr) newVal) {
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) BoolExpr::subst(std::string name, PTR(Expr) newVal) {
    return NEW(BoolExpr)(CAST(BoolExpr)(THIS)->val);
}
...
PTR(Val) IfExpr::interp() {
    PTR(Val) cond_val = cond->interp();
    if(!CAST(BoolVal)(cond_val)) {
        throw std::runtime_error("condition is not a boolean");
    }
    if(cond_val->is_true()) {
        return CAST(IfExpr)(THIS)->then_body->interp();
    } else {
        return CAST(IfExpr)(THIS)->else_body->interp();
    }
}
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) IfExpr::subst(std::string name, PTR(Expr) newVal) {
    return NEW(IfExpr)(CAST(IfExpr)(THIS)->cond->subst(name, newVal), CAST(IfExpr)(THIS)->then_body->subst(name, newVal), CAST(IfExpr)(THIS)->else_body->subst(name, newVal));
}
...
PTR(Val) EqExpr::interp() {
    // int n = rhs->interp();
    if(CAST(EqExpr)(THIS)->lhs->interp()->equals(CAST(EqExpr)(THIS)->rhs->interp())) {
        return NEW(BoolVal)(true);
    }
    else {
        return NEW(BoolVal)(false);
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) EqExpr::subst(std::string name, PTR(Expr) newVal) {
    return NEW(EqExpr)(CAST(EqExpr)(THIS)->lhs->subst(name, newVal), CAST(EqExpr)(THIS)->rhs->subst(name, newVal));
}
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) FunExpr::subst(std::string name, PTR(Expr) newVal) {
    if(name == arg) {
        return THIS;
    }
    return NEW(FunExpr)(arg, body->subst(name, newVal));
}
...
PTR(Val) CallExpr::interp() {
    // int n = body->interp();
    return fun->interp()->call(arg->interp());
}
 ...
 * \param rhs is our righthand side
 */
PTR(Expr) CallExpr::subst(std::string name, PTR(Expr) newVal) {
    return NEW(CallExpr)(CAST(CallExpr)(THIS)->fun->subst(name, newVal), CAST(CallExpr)(THIS)->arg->subst(name, newVal));
}
