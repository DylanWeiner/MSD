 ...
 * \param rhs is our righthand side
 */
Expr* FunExpr::subst(std::string name, Expr* newVal) {
    if(name == arg) {
        return this;
    }
...
* \brief Sets If values.
  * \param val is our boolean value
  */
CallExpr::CallExpr(Expr *fun, Expr *arg) {
    this->fun = fun;
    this->arg = arg;
}
 ...
 * \param rhs is our righthand side
 */
Expr* CallExpr::subst(std::string name, Expr* newVal) {
    return new CallExpr(this->fun->subst(name, newVal), this->arg->subst(name, newVal));
}
