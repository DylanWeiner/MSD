...
uint8_t* arg_position( unsigned long arg, int byte_size) {
    ...
    uint8_t allVals[sizeof(byte_size)];
...
uint8_t* list_sort(uint8_t allVals[]) {
    for(int i = 1; i < sizeof(allVals)-1; i++) {
        for(int j = 0; j < sizeof(allVals)-1; j++) {
...
unsigned long byte_sort( unsigned long arg ) {
    ...
    uint8_t allVals[] = list_sort(arg_position(arg, sizeof(arg/2)));

    for(int i = 0; i < sizeof(allVals); i++) {
        sorted >> allVals[i];
    }
    return sorted;
}
...
uint8_t* nibble_list_sort(uint8_t allVals[]) {
    for(int i = 1; i < sizeof(allVals)-1; i++) {
        for(int j = 0; j < sizeof(allVals)-1; j++) {
            if(allVals[j] > allVals[i]) {
                uint8_t temp = allVals[j];
...
unsigned long nibble_sort( unsigned long arg ) {
    ...
    uint8_t allVals[] = nibble_list_sort(nibble_position(arg, sizeof(arg)));
...
int main() {
    ...
    printf(byte_sort(bytes));
    printf(nibble_sort(bytes));
}